...
# CUDA 库符号链接设置脚本
# 在容器内运行此脚本来创建必要的 cuDNN、cuPTI 和 NCCL 符号链接
...
echo ""
echo "检查 libnccl.so.2:"
if [ -f "$NCCL_LIBS/libnccl.so.2" ]; then
    ls -la "$NCCL_LIBS/libnccl.so.2"
    echo "✓ libnccl.so.2 可用"
else
    echo "✗ libnccl.so.2 不可用"
fi
