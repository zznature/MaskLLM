#!/usr/bin/bash
# Llama8b MaskLLM 稀疏化训练脚本 - 修复版本
# 基于错误分析，调整为TP=4减少GPU需求
...
export MASTER_PORT="45524" # select a different port

# Device Configs - 修复GPU配置
NNODES=1 # number of nodes. 
NPROC_PER_NODE=4 # 减少到4个GPU避免映射冲突
export WORLD_SIZE=$(($NNODES * $NPROC_PER_NODE))
resume=$1 # resume from checkpoint

# 设置GPU可见性
export CUDA_VISIBLE_DEVICES=0,1,2,3  # 明确指定前4个GPU

# Task Configs
TAG="llama8b-tp4-mask-only-c4-test" # 更新为TP4
...
TOKENIZER_TYPE="NullTokenizer" # Use NullTokenizer for large vocab

TENSOR_PARALLEL_SIZE=4  # 减少到4以匹配可用GPU
PIPELINE_PARALLEL_SIZE=1
LR=5e-5
MIN_LR=5e-6
TRAIN_ITERS=50  # 进一步减少用于快速测试
WARMUP_ITERS=0
MICRO_BATCH_SIZE=1
GLOBAL_BATCH_SIZE=4 # 减少batch size匹配TP=4
...
if [ $resume -eq 0 ]; then
    # 需要转换为TP=4的checkpoint，或使用enable-partial-load
...
cd $PROJECT_PATH; mkdir -p $CKPT_SUBDIR/ckpt; mkdir -p $CKPT_SUBDIR/logs; export WANDB_API_KEY=$WANDB_API_KEY; echo Start Llama8b MaskLLM Training TP4

echo "🚀 Llama8b MaskLLM稀疏化训练测试 (TP=4修复版)"
echo "📋 配置信息:"
echo "  模型: Llama8b (119696 vocab)"
echo "  并行: TP=$TENSOR_PARALLEL_SIZE, PP=$PIPELINE_PARALLEL_SIZE"
echo "  GPU: $NPROC_PER_NODE GPUs (CUDA_VISIBLE_DEVICES=$CUDA_VISIBLE_DEVICES)"
