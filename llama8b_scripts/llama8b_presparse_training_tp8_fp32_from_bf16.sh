...
# Llama8b FP32训练脚本 - 从bf16 checkpoint恢复
# 特殊用途: 从bf16 iter 600恢复，继续fp32训练到2000
# 自动处理bf16→fp32精度转换

export MASTER_ADDR="127.0.0.1"
export MASTER_PORT="45534" # 独立端口，避免冲突
NNODES=1
NPROC_PER_NODE=8
...
# 路径配置
# 🔑 关键：从bf16 checkpoint加载
BF16_CHECKPOINT="$PROJECT_DIR/output/checkpoints/llama8b_maskllm_training_tp8"
FP32_CHECKPOINT="$PROJECT_DIR/output/checkpoints/llama8b_maskllm_training_tp8_fp32_from_bf16"
...
# 检查bf16 checkpoint
if [ ! -d "$BF16_CHECKPOINT" ]; then
    echo "❌ 错误: bf16 checkpoint不存在: $BF16_CHECKPOINT"
    echo "请先运行bf16训练或检查路径"
    exit 1
fi

# 检查iter 600是否存在
if [ ! -d "$BF16_CHECKPOINT/iter_0000600" ]; then
    echo "❌ 错误: iter_0000600不存在于: $BF16_CHECKPOINT"
    echo "可用的checkpoint:"
    ls -d "$BF16_CHECKPOINT"/iter_* 2>/dev/null || echo "  无checkpoint"
    exit 1
fi

echo "✅ 找到bf16 checkpoint: $BF16_CHECKPOINT/iter_0000600"
...
# 🔑 关键：从bf16 checkpoint加载，保存到fp32目录
LOAD_CHECKPOINT="$BF16_CHECKPOINT"
SAVE_CHECKPOINT="$FP32_CHECKPOINT"
mkdir -p "$SAVE_CHECKPOINT"

# 兼容性参数（关键！）
# --no-load-optim: 不加载优化器（重新初始化）
# --no-load-rng: 不加载RNG状态
# 不使用--finetune: 保留iteration计数，从601继续
EXTRA_CMD="$EXTRA_CMD --no-load-optim --no-load-rng"

echo "🔄 从bf16恢复到fp32模式:"
echo "  - 加载: $BF16_CHECKPOINT (bf16, iter 600)"
echo "  - 保存: $FP32_CHECKPOINT (fp32, iter 601+)"
echo "  - 自动精度转换: bf16 → fp32"
echo "  - 优化器: 重新初始化（前5次迭代重新适应）"

echo ""
echo "🎯 Llama8b FP32恢复训练配置:"
echo "  - 恢复方式: 从bf16 checkpoint自动转换"
echo "  - 精度: bf16 → FP32 (自动提升)"
echo "  - 显存优化: Activation Recomputation"
echo "  - bf16 checkpoint: $BF16_CHECKPOINT/iter_0000600"
echo "  - fp32保存目录: $FP32_CHECKPOINT"
echo "  - 数据路径: C4预处理"
echo "  - 稀疏率: ${SPARSITY} (50%)"
echo "  - 稀疏模式: ${PATTERN} (2:4结构化)"
echo "  - 起始迭代: 601 (继续自600)"
echo ""

# 训练参数
TRAIN_ITERS=2000
SAVE_INTERVAL=50  # 🔧 保持与bf16相同的保存频率
EVAL_INTERVAL=100
LOG_INTERVAL=1
WARMUP_ITERS=0

# 创建日志目录
LOG_DIR="$PROJECT_DIR/output/logs/llama8b_presparse_training_fp32_from_bf16"
mkdir -p "$LOG_DIR"
LOG_FILE="$LOG_DIR/training_from_bf16_${DATETIME}.log"

echo "📝 日志文件: $LOG_FILE"
echo ""
echo "🔍 恢复验证:"
echo "  1. 检查bf16 checkpoint完整性..."
BF16_FILES=$(ls "$BF16_CHECKPOINT"/iter_0000600/mp_rank_*/model_optim_rng.pt 2>/dev/null | wc -l)
echo "     找到 $BF16_FILES/8 个rank文件"
if [ $BF16_FILES -ne 8 ]; then
    echo "     ⚠️  警告: 期待8个文件，实际找到$BF16_FILES个"
fi
...
# - 完全避免 NaN
TASK_CMD="--gumbel-scale-range 1e2 5e2 --gumbel-temperature-range 6 2.0 --N 2 --M 4 --mask-only --prior-strength 3.0 --lr-mult 1 --weight-reg 1e-7 --clip-grad 0.3"
...
options=" \
    ...
    --log-diff-mask \
    --exit-signal-handler \
    --exp-name llama8b-tp8-mask-only-c4-fp32-from-bf16 \
    $EXTRA_CMD $TASK_CMD "
