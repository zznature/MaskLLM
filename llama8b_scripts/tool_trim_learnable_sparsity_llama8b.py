...
parser = argparse.ArgumentParser(description='Trim Llama8b checkpoint')
...
def trim_ckpt(input, output):
    ...
    for k,v in ckpt['model']['language_model']['encoder'].items():
        if '.diff_mask.gate' in k: 
            ...
            # set the type of winner mask to float16 for llama8b
            winner_mask = winner_mask.to(torch.float16)
        ...
        # Convert all weights to float16 for llama8b
