...
def parse_args(extra_args_provider=None, ignore_unknown_args=False):
    ...
    parser = _add_learnable_sparsity_args(parser)
...
def _add_network_size_args(parser):
    ...
    group.add_argument('--rotary-base', type=int, default=10000, help='Base for rotary embeddings.')
...
def _add_sparse_args(parser):
    """ for oneshot pruning
    """
    group = parser.add_argument_group(title="oneshot pruning")
    group.add_argument("--hessian-samples", type=int, default=128)
    group.add_argument(
        "--hessian-compute",
        action="store_true",
        help="Whether to computer Hessian. Hessian only needs" " to be computed once.",
    )
    group.add_argument(
        "--sparse-pattern",
        type=str,
        default="unstr",
        choices=["unstr", "nmprune"],
    )
    group.add_argument("--sparse-method", type=str, default="hessian")
    group.add_argument("--prunen", type=int, default=2)
    group.add_argument("--prunem", type=int, default=4)
    group.add_argument("--blocked-size", type=int, default=4)
    group.add_argument("--sparse-dir", type=str, default="")
    group.add_argument("--row-b", type=int, default=-1)
    group.add_argument("--col-b", type=int, default=128)
    group.add_argument("--hessian-damp", type=float, default=0.1)
    group.add_argument("--sparsity", type=float, default=0.5)
    group.add_argument("--offload", action="store_true")
    group.add_argument("--evaluate", action="store_true")
    group.add_argument("--fixed-sparse-mask", action="store_true")
    group.add_argument("--exclude-layers-from-prune", type=int, default=None)
    group.add_argument("--progressive", action="store_true")
    group.add_argument("--update-weight", action="store_true")
    group.add_argument("--freeze-mask", action="store_true")
    return parser


def _add_learnable_sparsity_args(parser):
    group = parser.add_argument_group(title="learnable sparsity")
    group.add_argument('--gumbel-scale-range', nargs=2, type=float, default=[1e3, 1e4], help='range of scaling factor for the mask')
    group.add_argument('--gumbel-temperature-range', nargs=2, type=float, default=[4, 0.05], help='range of scaling factor for the mask')
    group.add_argument('--log-diff-mask', action='store_true', default=False, help='log differentible mask during training')
    group.add_argument('--exp-name', type=str, default="sparsity",
                        help="experiment name")
    group.add_argument('--wandb-id', type=str, default=None,
                        help="experiment name") 
    group.add_argument('--enable-partial-load', action='store_true', default=False,
                        help='enable partial load')
    group.add_argument('--N', default=2, type=int, help='N:M sparsity')
    group.add_argument('--M', default=4, type=int, help='N:M sparsity')
    group.add_argument('--hard', action='store_true', default=False, help='hard mask')
    group.add_argument('--mask-only', action='store_true', default=False, help='mask-only training')
    group.add_argument('--prior-strength', default=0.0, type=float, help='prior strength')
    group.add_argument('--weight-reg', type=float, default=0.0, help='sparse regularization loss')

    group.add_argument('--lr-mult', type=float, default=5.0, help='learning rate multiplier for mask')
    group.add_argument('--skip-layers', action='store_true', default=False, help='update layer norm parameters')
    group.add_argument('--target-layer-for-sensitivity-analysis', default=None, type=int, help="LAYERID_LINEARID")
    group.add_argument('--kd-temperature', default=1, type=float, help='temperature for KD')
    group.add_argument('--enable-sparsity', action='store_true', default=False, help='update layer norm parameters')
    group.add_argument('--iteration-offset', default=0, type=int, help='iteration offset in wandb visualization')
    return parser