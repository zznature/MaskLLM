...
def scale_lr_cond_gate(name, param):
    if 'diff_mask.gate' in name:
        return True
    return False
...
def pretrain(train_valid_test_dataset_provider,
    ...
    if is_last_rank():
        if args.exp_name is not None:
            wandb_resume = os.path.join(args.save, args.exp_name+'.wandb_id')
            if os.path.exists(wandb_resume):
                ...
                with open(wandb_resume, 'w') as f:
                    f.write(wandb_id)
            wandb.init(project="Megatron-MaskLLM", name=args.exp_name, id=wandb_id, resume="allow", config=get_args())
        else:
            wandb.init(project="Megatron-MaskLLM", name=args.exp_name, config=get_args())
    ...
    model, optimizer, opt_param_scheduler = setup_model_and_optimizer(
        model_provider, model_type, scale_lr_cond=scale_lr_cond_gate, lr_mult=args.lr_mult) # 10x learning rate for diff_mask.gate
...
def training_log(loss_dict, total_loss_dict, learning_rate, iteration,
    ...
    if iteration % args.log_interval == 0:
        ...
        if is_last_rank():
            log_dict = {
                'train/iteration': iteration+args.iteration_offset,
                'train/consumed_train_samples': args.consumed_train_samples,
                'train/elapsed_time_per_iteration': elapsed_time_per_iteration,
        ...
        for key in total_loss_dict:
            if key not in [advanced_iters_key, skipped_iters_key,
                ...
                if is_last_rank(): log_dict['train/' + key] = avg
                total_loss_dict[key] = torch.tensor([0.0], dtype=torch.float, device='cuda')

        log_string += ' loss scale: {:.1f} |'.format(loss_scale)
        if grad_norm is not None:
            log_string += ' grad norm: {:.3f} |'.format(grad_norm)
            if is_last_rank(): log_dict['train/grad_norm'] = grad_norm
        if num_zeros_in_grad is not None:
            log_string += ' num zeros: {:.1f} |'.format(num_zeros_in_grad)
            if is_last_rank(): log_dict['train/num_zeros'] = num_zeros_in_grad
        if params_norm is not None:
            log_string += ' params norm: {:.3f} |'.format(params_norm)
            if is_last_rank(): log_dict['train/params_norm'] = params_norm
        log_string += ' number of skipped iterations: {:3d} |'.format(
            total_loss_dict[skipped_iters_key])
        if is_last_rank(): log_dict['train/skipped_iterations'] = total_loss_dict[skipped_iters_key]
        log_string += ' number of nan iterations: {:3d} |'.format(
            total_loss_dict[nan_iters_key])
        if is_last_rank(): log_dict['train/nan_iterations'] = total_loss_dict[nan_iters_key]
        total_loss_dict[advanced_iters_key] = 0
        total_loss_dict[skipped_iters_key] = 0
        total_loss_dict[nan_iters_key] = 0
        if diff_mask_dict is not None:
            for key in diff_mask_dict:
                log_string += ' {}: {:.3f} |'.format(key, diff_mask_dict[key])
                if is_last_rank(): log_dict['train/' + key] = diff_mask_dict[key]
...
def collect_diff_mask_dict(model):
    diff_mask_dict = {}
    args = get_args()
    with torch.no_grad():
        for name, module in model[0].named_modules():
            if hasattr(module, 'diff_mask'): 
                diff_mask = module.diff_mask
                diff_mask_dict['scale_multiplier'] = diff_mask.current_scale_multiplier
                diff_mask_dict['temperature'] = diff_mask.current_temperature
                # diff_mask N x 6
                max_prob = gumbel_softmax(diff_mask.gate *  diff_mask_dict['scale_multiplier'], tau=diff_mask_dict['temperature'], hard=False).max(-1)[0]
                max_prob = max_prob.mean()
                diff_mask_dict['max_prob'] = max_prob
                diff_mask_dict['mask_difference'] = diff_mask.mask_difference
                diff_mask_dict['logits_mean'] = diff_mask.gate.mean()
                diff_mask_dict['logits_std'] = diff_mask.gate.std()
                break
            
    assert len(diff_mask_dict) > 0, "No diff mask found"
    return diff_mask_dict
...
def train(forward_step_func, model, optimizer, opt_param_scheduler,
    ...
    for m in model[0].modules():
        if hasattr(m, 'diff_mask'):
            diff_masks.append(m.diff_mask)
    print_rank_0(f"Number of diff masks: {len(diff_masks)}")
    ...
    while iteration < args.train_iters:
        ...
        diff_mask_dict = None
        if args.log_diff_mask:
            diff_mask_dict = collect_diff_mask_dict(model)
...
def evaluate_and_print_results(prefix, forward_step_func,
    ...
    wandb_log = {'iteration': iteration+args.iteration_offset}
    for key in total_loss_dict:
        string += '{} value: {:.6E} | '.format(key, total_loss_dict[key].item())
        ppl = math.exp(min(20, total_loss_dict[key].item()))
        string += '{} PPL: {:.6E} | '.format(key, ppl)
        wandb_log[f"{key} value"] = total_loss_dict[key].item()
        wandb_log[f"{key} PPL"] = ppl
        if is_last_rank():
            try:
                wandb.log(wandb_log)
            except: pass
